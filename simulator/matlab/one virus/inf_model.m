function [dxdt,dxbind_dt,dxnucl_dt] = inf_model(t,x,x_bind,x_nucl,...
    ...
    dxdt(end-1)=dxdt(end-1)+k_deathT*T;
    ...
    %% I1: infection, death, and intracellular species balances
    for j = 1:n_bins_inf
        age=age_bins(j);
        ...
        dxnucl_dt(end)=dxnucl_dt(end)+k_deathI1*x_nucl(j);
    end
    ...
    dxdt(end-1)=dxdt(end-1)-(k_lys+b)*NV;
    dxnucl_dt(end)=dxnucl_dt(end)-(k_lys+b+k_d_N1)*x_nucl(end);

    % substrate consumption
    dxdt(end)=D*(Sin-S)-(Ys_T*T+Ys_I1*totI1)*S/(1e-2/scaling+S);
