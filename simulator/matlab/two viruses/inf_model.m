function [dxdt,dxbind_dt,dxnucl_dt] = inf_model(t,x,x_bind,x_nucl,...
    ...
    dxdt(endCo+1)=dxdt(endCo+1)+k_deathT*T;

    dxdt(startI1)=dxdt(startI1)+bind1; % infected cells balance
    dxdt(startI2)=dxdt(startI2)+bind2; % infected cells balance
    ...
    for j = 1:n_bins_inf
        ...
        dxdt(startI1+j-1)=dxdt(startI1+j-1)+growthI1;
        dxdt(startI2+j-1)=dxdt(startI2+j-1)+growthI2;
        ...
        dxdt(startI1+j-1)=dxdt(startI1+j-1)-deathI1;
        dxdt(startI2+j-1)=dxdt(startI2+j-1)-deathI2;
        dxdt(endCo+1)=dxdt(endCo+1)+deathI1+deathI2;
        ...
        dxdt(startI1+j-1)=dxdt(startI1+j-1)-B2_new;  % remove new co-infected cells from I1 bal
        dxdt(startI2+j-1)=dxdt(startI2+j-1)-B1_new;  % remove new co-infected cells from I2 bal

        dxdt(startCo-1+ind(1+n_bins*(j-1)))=dxdt(startCo-1+ind(1+n_bins*(j-1)))+B2_new; % add new co-infected cells to Co bal
        dxdt(startCo-1+j)=dxdt(startCo-1+j)+B1_new; % add new co-infected cells to Co bal

        v1_toCoB=kv2*x_bind(j); % V1 bound to I1 moving to co-infected cells
        v2_toCoB=kv1*x_bind(startB2+j-1); % V2 bound to I2 moving to co-infected cells
        ...
        dxbind_dt(j)=dxbind_dt(j)-v1_toCoB; % move V1 bound to I1 that just got co-infected from B/I1 to B/Co balance
        dxbind_dt(startB2+j-1)=dxbind_dt(startB2+j-1)-v2_toCoB;  % move V2 bound to I2 that just got co-infected from B/I2 to B/Co balance  
        dxnucl_dt(j)=dxnucl_dt(j)-v1_toCoN; % move V1 bound to I1 that just got co-infected from B/I1 to B/Co balance
        dxnucl_dt(startB2+j-1)=dxnucl_dt(startB2+j-1)-v2_toCoN;  % move V2 bound to I2 that just got co-infected from B/I2 to B/Co balance  

        dxbind_dt(startB1Co-1+j)=dxbind_dt(startB1Co-1+j)+B1_new; % update V1 bound to new co-infected cells
        dxbind_dt(ind_scB1Co(1+n_bins*(j-1)))=dxbind_dt(ind_scB1Co(1+n_bins*(j-1)))+... % update V1 bound to new co-infected cells
            v1_toCoB;
        dxnucl_dt(ind_scB1Co(1+n_bins*(j-1)))=dxnucl_dt(ind_scB1Co(1+n_bins*(j-1)))+... % update E1 to new co-infected cells
            v1_toCoN;

        dxbind_dt(startB2Co-1+j)=dxbind_dt(startB2Co-1+j)+v2_toCoB; % update V2 bound to new co-infected cells
        dxnucl_dt(startB2Co-1+j)=dxnucl_dt(startB2Co-1+j)+v2_toCoN; % update E2 bound to new co-infected cells
        dxbind_dt(ind_scB2Co(1+n_bins*(j-1)))=dxbind_dt(ind_scB2Co(1+n_bins*(j-1)))+B2_new; % update V2 bound to new co-infected cells
            
        % virus of the same type enters I1 and I2
        I1_uptake=k_bind_I1(j)*I1(j)*V1; % calculate virus uptake
        I2_uptake=k_bind_I2(j)*I2(j)*V2;
        dxbind_dt(j)=dxbind_dt(j)+I1_uptake; % add uptake to bound virus balance
        ...
        % bleeding
        dxdt(startI1+j-1)=dxdt(startI1+j-1)-b*I1(j);
        dxdt(startI2+j-1)=dxdt(startI2+j-1)-b*I2(j);
    ...
    for k=1:length(Co)
        ...
        %% Growth
        growthCo=mu_Co*Co(k)*growth_lim;
        dxdt(startCo+k-1)=dxdt(startCo+k-1)+growthCo;
        ...
        dxdt(startCo-1+k)=dxdt(startCo-1+k)-deathCo;
        dxdt(endCo+1)=dxdt(endCo+1)+deathCo;
        ...
        % bleeding
        dxdt(startCo-1+k)=dxdt(startCo-1+k)-b*Co(k);
