function [dxdt,dxbind_dt,dxnucl_dt] = inf_model_FD(t,x,x_bind,x_nucl,...
    ...
    % fluxes
    Dtau=age_bins(2)-age_bins(1);
    
    dI1dt(1)=dI1dt(1)-I1(1)/Dtau;
    dI1dt(2:n_bins-1)=dI1dt(2:n_bins-1)-(I1(2:n_bins-1)-I1(1:n_bins-2))/Dtau;
    dI1dt(end)=dI1dt(end)+I1(end-1)/Dtau;

    dxbind_dt(1)=dxbind_dt(1)-x_bind(1)/Dtau;
    dxbind_dt(2:n_bins-1)=dxbind_dt(2:n_bins-1)-(x_bind(2:n_bins-1)-x_bind(1:n_bins-2))/Dtau;
    dxbind_dt(end)=dxbind_dt(end)+x_bind(end-1)/Dtau;

    dxnucl_dt(1)=dxnucl_dt(1)-x_nucl(1)/Dtau;
    dxnucl_dt(2:n_bins-1)=dxnucl_dt(2:n_bins-1)-(x_nucl(2:n_bins-1)-x_nucl(1:n_bins-2))/Dtau;
    dxnucl_dt(end)=dxnucl_dt(end)+x_nucl(end-1)/Dtau;
